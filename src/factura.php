...
$factura->get('/fecha/lista', function () use ($app) {
  //STR_TO_DATE('17-09-2010','%d-%m-%Y')
  $data = json_decode( file_get_contents("php://input"), true );
  $sql = "select * from tt_factura where fecha_inicio between ? and ? order by factura_id desc";
  return $app->json( $app['db']->fetchAll( "SELECT * FROM tt_factura order by factura_id desc", array() ) );
});
...
$factura->get('/proforma/{id}', function ($id) use ($app) {
  $sql = "select d.proforma_id, d.fecha_inicio, d.fecha_fin, d.serie, d.numero_proforma, "
        ."d.numero_envio, d.estado, d.caja_id, d.total_producto, d.total, d.iva, d.descuento, d.total_sin_iva, "
        ."d.cliente_id, d.usuario_id, d.tipo_id, if(d.tipo_id = 1, d.numero_proforma, d.numero_envio) numero_factura, "
        ."p.cliente_desc, p.nit, p.direccion "
        ."from tt_proforma d "
        ."inner join tc_cliente p on d.cliente_id = p.cliente_id "
        ."where d.proforma_id = ?";
  $data = $app['db']->fetchAssoc( $sql, array($id));
...
$factura->get('/proforma/tipo/{tipo}/lista', function () use ($app) {
  $sql = "select d.proforma_id, d.fecha_inicio, d.fecha_fin, d.serie, d.numero_proforma, "
        ."d.numero_envio, d.estado, d.caja_id, d.total_producto, d.total, d.iva, d.descuento, d.total_sin_iva, "
        ."d.cliente_id, d.usuario_id, d.tipo_id, if(d.tipo_id = 1, d.numero_proforma, d.numero_envio) numero_factura, "
        ."p.cliente_desc, p.nit, p.direccion "
...
$factura->post('/add', function () use ($app) {
  $data = json_decode( file_get_contents("php://input"), true );
  try {
    $respuesta = null;
    ...
    if ($continuar == true) {
      ...
      if ($continuar) {
        ...
        if ($totalCredito > 0) {
          $app['db']->insert('tt_cuenta_cobrar', array(
            'cuenta_cobrar_desc' => 'Crédito al cliente '.$data['cliente_id'].' por: '.$totalCredito,
            'valor' => $totalCredito,
            'saldo' => $totalCredito,
            'estado' => 1,
            'fecha_registro' => date('Y-m-d H:i:s'),
      ...
      $respuesta = $app->json(resultArray('error', 'El monto pagado no cubre el total de la factura', $totalFactura." - ".$totalPagado));
    }
    if ($hacerCommit) {
      $app['db']->commit();
    } else {
...
$factura->post('/proforma/add', function () use ($app) {
  ...
  try {
    ...
    //obtener correlativo caja
    $sql = "";
    if ( $data['tipo'] == 1 ) { //si es proforma
        $sql = "select numero_proforma + 1 numero_proforma, serie from tc_caja where caja_id = ".$data['caja_id']." and estado = 1";
    } else { //de lo contrario envio
        $sql = "select numero_envio + 1 numero_envio, serie from tc_caja where caja_id = ".$data['caja_id']." and estado = 1";
    }
    $caja = $app['db']->fetchAll($sql, array());
    ...
    $numeroEnvio = 0;
    if ( $data['tipo'] == 1 ) { //si es proforma
      $app['db']->update( 'tc_caja', array('numero_proforma' => $caja[0]['numero_proforma']), array( 'caja_id' => $data['caja_id'], 'estado' => 1 ) );
      $numeroProforma = $caja[0]['numero_proforma'];
    } else {
      $app['db']->update( 'tc_caja', array('numero_envio' => $caja[0]['numero_envio']), array( 'caja_id' => $data['caja_id'], 'estado' => 1 ) );
      $numeroEnvio = $caja[0]['numero_envio'];
    }
